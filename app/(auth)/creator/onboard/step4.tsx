...
import { apiErrorMessage, uploadToStorage } from '@/services';
...
interface CertDocument {
  ...
  label: string;
  description: string;
  /** Local URI shown for preview; replaced by the stored object URL on upload success. */
  uri: string | null;
  uploading?: boolean;
}
...
export default function CreatorOnboardStep4() {
  const { theme } = useTheme();
  const styles = useStyles(makeStyles);
  const router = useRouter();
  const { certifications, upsertCert, removeCert } = useCreatorOnboardStore();
    ...
    <ScreenWrapper scrollable statusBarStyle="dark">
      ...
      <View style={styles.body}>
        ...
        <Button
          ...
          onPress={() => router.push('/(auth)/creator/onboard/step5')}
        />
      </View>
    </ScreenWrapper>
  );
