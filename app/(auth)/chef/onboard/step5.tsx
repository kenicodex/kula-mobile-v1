...
export default function ChefOnboardStep5() {
  const styles = useStyles(makeStyles);
  const router = useRouter();
  const onboard = useChefOnboardStore();
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async () => {
      // 1) Create the chef profile from data collected across step1-4.
      await chefsService.createProfile({
        bio: onboard.bio,
