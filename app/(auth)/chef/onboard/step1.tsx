...
export default function ChefOnboardStep1() {
  const { theme } = useTheme();
  const styles = useStyles(makeStyles);
  const router = useRouter();
  const { user, updateUser } = useAuthStore();
  const { avatarUrl, bio, city, setAvatar, setStep1 } = useChefOnboardStore();
  ...
  const pickPhoto = async () => {
    ...
    try {
      ...
      try {
        ...
        // Non-fatal — Cloudinary URL is still in the onboard store and will
        // be used when the chef profile is finalised.
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    // resubmitted with the chef profile create.
    ...
    router.push('/(auth)/chef/onboard/step2');
  };
